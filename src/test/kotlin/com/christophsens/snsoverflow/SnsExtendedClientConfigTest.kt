...
class SnsExtendedClientConfigTest {
    private val payloadStore = mockk<PayloadStore>()

    @Test
    fun `defaults are sensible`() {
        val config = SnsExtendedClientConfig(payloadStore)
    ...
    fun `trims the s3 key prefix`() {
        val config = SnsExtendedClientConfig(payloadStore, s3KeyPrefix = "  my-prefix  ")
    ...
    fun `rejects an s3 key prefix that is too long`() {
        assertThatThrownBy { SnsExtendedClientConfig(payloadStore, s3KeyPrefix = "a".repeat(MAX_S3_KEY_PREFIX_LENGTH + 1)) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `rejects an s3 key prefix starting with a dot`() {
        assertThatThrownBy { SnsExtendedClientConfig(payloadStore, s3KeyPrefix = ".hidden") }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `rejects an s3 key prefix starting with a slash`() {
        assertThatThrownBy { SnsExtendedClientConfig(payloadStore, s3KeyPrefix = "/absolute") }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `rejects an s3 key prefix containing a double dot`() {
        assertThatThrownBy { SnsExtendedClientConfig(payloadStore, s3KeyPrefix = "a/../b") }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `rejects an s3 key prefix with invalid characters`() {
        assertThatThrownBy { SnsExtendedClientConfig(payloadStore, s3KeyPrefix = "not valid!") }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
}
